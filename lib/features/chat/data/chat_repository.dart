...
class ChatRepository {
  ...
  Stream<List<ChatMessage>> messagesStream({String? dialogId, required String role}) {
    ...
    // Prefer dialog-specific stream; otherwise keep a role-based stream (currently unused).
    final Query q = (dialogId != null && dialogId.isNotEmpty)
        ? col.where('dialogId', isEqualTo: dialogId)
        : col.where('role', isEqualTo: role);
